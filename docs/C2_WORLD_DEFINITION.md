...
**C3 пересматривает эту абстракцию:** текущий универсальный core строится как геномный калькулятор над обычными числовыми значениями. При конфликте с разделами ниже приоритет имеет [`C3_CALCULATOR_MODEL.md`](C3_CALCULATOR_MODEL.md).
...
Пользовательские инструменты и renderer должны показывать имя типа из world definition. Машинные идентификаторы
`Field[n]`, `Resource[n]`, `State[n]`, `Matter[n]`, `Measure[n]` и `Property[n]` являются внутренней адресацией и
используются в пользовательском выводе только как fallback/debug-представление, если имя не задано.
