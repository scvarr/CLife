...
Наблюдение: сейчас `Value` содержит stable ID и имя, а `Amount` сам по себе не сообщает, что именно означает число. Ранняя C2-модель исследовала похожую область через `Measure` и `measure_per_unit`, но C2 историческая; её API не должен восстанавливаться автоматически.

Возможное направление: Value получает описание единицы количества. `WorldDefinition` получает отдельные данные для базовых масштабов, производительности на такт и отношений преобразования между различными Value. Genome parameters преимущественно остаются относительными коэффициентами.

Будущий UI создания Value, вероятно, недостаточно описывать только как `Имя: Свет; + Значение`: он мог бы позволить указать `Единица: L`. Отношения не нужно задавать для каждой пары Value; потенциально UI мог бы разделять:
...
Пока НЕ решено: нужен ли полноценный dimension type system; будет ли `UnitDefinition` отдельной сущностью; единица принадлежит Value или общей Dimension; как задаются производные единицы и automatic conversions; как expression compiler проверяет размерности; как units сериализуются.
